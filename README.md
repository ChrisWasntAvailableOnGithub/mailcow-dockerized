...
## ApolloMC branding + quick setup tutorial
...
4. Open **Email > Configuration > Mail Setup** and add your domain (example: `example.com`).
5. In your DNS provider, add these records for that domain:
...
6. Wait for DNS propagation, then use **System > Tools > DNS Checks** (or similar diagnostics in UI) to verify records.
7. Create accounts under **Email > Configuration > Mailboxes**:
   - Click add mailbox
   - Set local part (e.g. `info`)
   - Select domain
   - Set password/quota and save
8. Test login in webmail (SOGo) and with an email client:
   - IMAP: port 993 (SSL/TLS)
   - SMTP submission: port 587 (STARTTLS) or 465 (TLS)
   - Username: full email address
9. Send a test mail to/from an external mailbox and confirm SPF/DKIM/DMARC pass in message headers.
10. Optional hardening: enforce 2FA for admins and review spam/ratelimit settings after first successful delivery.
